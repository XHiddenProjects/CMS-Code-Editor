<?php 
	...
	//setting: error report on or off
...
<?php  
	//current directory where the editor php file is stored
	$currentDir = __DIR__;
...
<?php  
	//code for ajax request of the set base function at clicking a folder
	...
	//code for ajax request of the filemanager
	...
	//ajax request for the file content loading into the editor
...
<?php 
	...
	if(isset($_POST['submitPassword'])){
		if(md5($_POST['password']) == md5(simplexml_load_file('libs/auth/credentals.xml')->psw)){
			$_SESSION["login"] = "true";
		}else{
...
<?php 
	...
	function dirListSidebar($dir){
		...
		foreach ($cleanDir as $name) {
			...
			// The following two lines of code are used to scan the directory recursively
...
<head>
	...
	<style>
		...
		.logo:hover {
            transform: scale(1.2);
            transition: all 1500ms cubic-bezier(0.6, -0.28, 0.74, 0.05);
        }
        .footer-copyright {
            padding: 2px;
...
<body>
	<div id="login_body">
		<div id="login_form_wrapper">
			<!--centered logo-->
	        <img class="logo" width="300" src="libs/cms_t_editor_logo.png">
			<form action="" method="post">
				...
				<input type="submit" value="Join" id="password_button" name="submitPassword">
			</form>
		</div>
	</div>
	<div class="footer-copyright text-center">
        <p class="text">© 2020-2022 by Andreas Holzer. Crafted by <a href="https://github.com/webcrew" rel="nofollow">WebCrew</a> All Rights Reserved.</p>
    </div>
</body>
</html>
...
<?php 
		...
		?>
		<!-- html for the select base folder page -->
		<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
...
<body>
	<div id="select_base_body">
		<div><h2>Select Your Base Folder / Project Folder</h2><h5>Here you should select the base folder or project folder, which you will then see in the editor sidebar.</h5></div>
		...
		<div id="base_form_wrapper">
			<form action="" onsubmit="return false;">
				...
				<input type="submit" value="Back" id="back_button" data-path="<?php echo backurl($currentDir, $seprator); ?>">
			</form>
	...
	<script>
		...
		function loadDir(path){
		    ...
		    xmlhttp.open("POST", "cms-t-editor.php", true); 
	...
	//code for file manager	
		...
		?>
			<!-- html for the file Manager-->
		<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<title>CMS-T-Editor - File Manager</title>
	<link href="https://fonts.googleapis.com/css?family=Source+Sans+Pro" rel="stylesheet">
	<link rel="stylesheet" href="libs/fontawesome/css/all.css">
	<link rel="apple-touch-icon" sizes="180x180" href="libs/apple-touch-icon.png">
    <link rel="icon" type="image/png" sizes="32x32" href="libs/favicon-32x32.png">
...
<body>
	...
	<div id="select_base_body">
		...
		<div id="base_form_wrapper">
			<form action="" onsubmit="return false;">
				<input type="hidden" value="">
				<input type="submit" value="Back" id="back_button" data-path="<?php echo backurl($baseFolder, $seprator); ?>">
			</form>
	...
	<script>
		...
		function addEventListrnerToLi(){
			...
			//loop throught all the li
			for (var i = 0; i < getInDir.length; i++) {
		 		//adding click event listener to all li	
		  		getInDir[i].addEventListener("click", function() {
		...
		// calling event listener for the first time
		addEventListrnerToLi();
		...
		// this part will contain all the code of editor
...
<div>
    ...
    <div id="editor" style="float: left; margin-top: 30px; margin-left: 200px;"></div>
</div>
<!-- load emmet code and snippets compiled for browser -->
<script src="libs/emmet.js"></script>
<!-- loading ace editor      -->
<script src="libs/ace/src-noconflict/ace.js" type="text/javascript" charset="utf-8"></script>
<script src="libs/ace/src-noconflict/ext-emmet.js"></script>
<script>
	// cookie function
...
<script>
	...
	//Load the folder contents into the sidebar by clicking on the folder name
	function loadDirAjax(e){
		//check if dir is alreay loaded and if it isloaded close it
	...
	//to set the language mode dynamically
